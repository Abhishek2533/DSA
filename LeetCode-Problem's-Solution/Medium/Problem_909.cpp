...
public:
    int snakesAndLadders(vector<vector<int>>& board) {
        ...
        // Convert the board into a 1D array following the Boustrophedon order
        for (int i = n - 1; i >= 0; --i) {
            if (leftToRight) {
                for (int j = 0; j < n; ++j) {
        ...
        while (!q.empty()) {
            ...
            // Roll the dice (up to 6)
            for (int dice = 1; dice <= 6; ++dice) {
                int nextPos = pos + dice;
                if (nextPos > n * n) continue; // Out of bounds
                
                // If there's a snake or ladder, move directly to its destination
                if (flattened[nextPos] != -1) nextPos = flattened[nextPos];
