...
public:
    int smallestChair(vector<vector<int>>& times, int targetFriend) {
        int targetArrivalTime = times[targetFriend][0];   // [arrival, departure] -> we need arrival time
        ...
        // Min-heap for occupied chairs -> [leavingTime, chairNo]
        ...
        for (const vector<int>& time : times) {
            ...
            // Free the chairs that have been vacated
            ...
            int currentChairNo;
            if (available.empty()) {
                // Assign a new chair if no available chair
                currentChairNo = chairNo;
                chairNo++;
            } else {
                // Reuse the smallest available chair
                currentChairNo = available.top();
                available.pop();
            }

            // Check if the current friend is the target friend
            if (targetArrivalTime == arrTime) return currentChairNo;

            // Mark the chair as occupied until this friend's leaving time
            occupied.push({leavingTime, currentChairNo});
        }

        return -1; // In case something goes wrong (though this line shouldn't be reached)
    }
};